...
// Implements encoding of CashTxDestination using cashaddr.
class CashAddrEncoder : public boost::static_visitor<std::string> {
public:
    explicit CashAddrEncoder(const CashParams &p) : params(p) {}
...
std::string EncodeCashAddr(const CashTxDestination &dst,
                           const CashParams &params) {
