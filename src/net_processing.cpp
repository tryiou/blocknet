...
static void PushNodeVersion(CNode *pnode, CConnman* connman, int64_t nTime)
{
    ...
    connman->PushMessage(pnode, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::VERSION, PROTOCOL_VERSION, (uint64_t)nLocalNodeServices, nTime, addrYou, addrMe,
            nonce, strSubVersion, nNodeStartingHeight, ::g_relay_txes, static_cast<bool>(pnode->fXRouter)));
...
static bool TipMayBeStale(const Consensus::Params &consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    return g_last_tip_update < GetTime() - consensusParams.nPowTargetSpacing * 10 && mapBlocksInFlight.empty();
}
    ...
    if (send && (pindex->nStatus & BLOCK_HAVE_DATA))
    {
        ...
        if (pblock) {
            if (inv.type == MSG_BLOCK) {
                if (pfrom->nVersion <= LEGACY_PROTOCOL_VERSION) // support legacy nodes
                    connman->PushMessage(pfrom, msgMaker.Make(SERIALIZE_TRANSACTION_NO_WITNESS, NetMsgType::BLOCK, CBlockLegacy(*pblock)));
                else
