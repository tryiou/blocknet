...
# Make /usr/bin and /bin for pure container (tolerant for read-only /)
[ -e /usr/bin ] || mkdir -p /usr/bin 2>/dev/null || true
[ -e /bin ] || mkdir -p /bin 2>/dev/null || true
# Symlink file, env, pwd, sh to conventional paths (tolerant)
[ -e /usr/bin/file ] || ln -s --no-dereference "$(command -v file 2>/dev/null || echo /gnu/store/*-coreutils-*/bin/file)" /usr/bin/file 2>/dev/null || true
[ -e /usr/bin/env ]  || ln -s --no-dereference "$(command -v env 2>/dev/null || echo /gnu/store/*-coreutils-*/bin/env)"  /usr/bin/env 2>/dev/null || true
[ -e /bin/pwd ] || ln -s --no-dereference "$(command -v pwd 2>/dev/null || echo /gnu/store/*-coreutils-*/bin/pwd)" /bin/pwd 2>/dev/null || true
[ -e /bin/sh ] || ln -s --no-dereference "$(command -v bash 2>/dev/null || command -v sh 2>/dev/null)" /bin/sh 2>/dev/null || true
...
export TAR_OPTIONS="--owner=0 --group=0 --numeric-owner --mtime='@${SOURCE_DATE_EPOCH}' --sort=name --no-same-owner --no-same-permissions"
export TZ="UTC"
case "$HOST" in
    *darwin*)
...
(
    ...
    tar --no-same-owner --no-same-permissions --strip-components=1 -xf "${GIT_ARCHIVE}"
    ...
    make -C src --jobs=1 check-symbols  ${V:+V=1} || echo "warning: check-symbols failed (non-fatal)"
