...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_blocknet
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_blocknet --log_level=test_suite
$ valgrind -v --leak-check=full src/blocknetd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_blocknet.coverage/index.html`.
```
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `blocknet-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `blocknet-cli` and the GUI debug console use this table to determine how to
